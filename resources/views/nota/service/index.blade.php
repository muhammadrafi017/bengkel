...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">Daftar Nota Service</h1>
</div>
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.payment-button', function() {
            let id = $(this).val();
            $.ajax({
                url: '{{ url('nota/service/payment') }}/' + id,
                type:'post',
                beforeSend: function() {
        ...
        $(document).on('click', '.take-button', function() {
            let id = $(this).val();
            $.ajax({
                url: '{{ url('nota/service/taking') }}/' + id,
                type:'post',
                beforeSend: function() {
                    $('button').prop('disabled', true);
                }, success: function(x) {
        ...
        $('.report').on('click', function() {
            ...
            if (start_date <= end_date) {
                ...
                window.open('{{ url('nota/service/report') }}/'+ content +'/'+ start_date +'/'+ end_date);
            } else {
                Swal.fire({
                    type: 'error',
                    title: 'Oops..',
        ...
        function generateTable() {
            table = $('#datatable').DataTable({
                ...
                destroy: true,
                ajax: {
                    url: '{{ url('datatable/nota-service') }}',
                    type:'POST',
                    data:function(d) {
